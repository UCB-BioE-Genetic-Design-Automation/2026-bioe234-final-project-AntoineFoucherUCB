...
class QuestionnaireParsing:
    ...
    def run(self, stage: str, raw_data: str) -> dict:
        try:
            ...
            if stage_key == "project_registration":
                # Header-only checkpoint: resets session storage; ignores PI-bearing keys intentionally.
                _reset_state()
                uni, st, bua_num, proj = _registration_from_data(data)
                current_bua_state.university = uni
                current_bua_state.state = st
                current_bua_state.bua_number = bua_num
                current_bua_state.project_title = proj
                current_bua_state.pi_info = ContactInfo()
                next_stage = "project_and_pi"

            elif stage_key == "project_and_pi":
                _maybe_patch_registration_fields(data)
                incoming_pi = _coerce_pi_contact_only(data)
                current_bua_state.pi_info = _merge_contact_pick_nonempty(current_bua_state.pi_info, incoming_pi)
