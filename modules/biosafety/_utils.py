...
def convert_to_markdown(file_path: str) -> str:
    """
    Reads a document of various types (DOCX, PDF, XLSX, PPTX, etc.) 
    and converts it to true Markdown, preserving tables and lists.
    """
    ...
    # Plain text files can still be read directly
    ...
    # For everything else (DOCX, PDF, etc.), use MarkItDown
    try:
        md = MarkItDown()
        result = md.convert(file_path)
        
        # Add a nice header so the LLM knows the filename
        final_markdown = f"## Document: {os.path.basename(file_path)}\n\n"
        final_markdown += result.text_content
        
        return final_markdown
        
    except ImportError:
        logging.error("markitdown library is missing.")
        return "Error: Missing 'markitdown' library. Please run 'pip install markitdown'."
    except Exception as e:
        logging.error(f"Error converting document {file_path} to Markdown: {e}")
        raise
...
class AbsaRetrieve:
    def initiate(self) -> None:
        self.csv_path = Path(__file__).resolve().parent.parent / "data" / "absa_db.csv"
        self.rows: list[dict] = []
...
_absa_searcher = AbsaRetrieve()
_absa_searcher.initiate()

def get_absa_data(scientific_name: str, top_k: int = 5) -> dict:
    """
    Standard Python function to fetch ABSA data.
    Can be imported and called by any other Python file in your project.
    """
    return _absa_searcher.run(query=scientific_name, top_k=top_k)
